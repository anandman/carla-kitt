...
sys.path.append(os.path.join(os.path.dirname(__file__), "../../tensorflow/research"))
sys.path.append(os.path.join(os.path.dirname(__file__), "../../tensorflow/research/slim"))
...
flags = tf.app.flags
flags.DEFINE_string('input_path', '', 'Path to yaml records')
...
def main(_):
    output_dirname = os.path.dirname(FLAGS.output_path)
    if not os.path.exists(output_dirname):
        os.makedirs(output_dirname)
    writer = tf.python_io.TFRecordWriter(FLAGS.output_path)

    # BOSCH
    examples = yaml.load(open(FLAGS.input_path, 'rb').read())
    ...
    for i in range(len(examples)):
        examples[i]['path'] = os.path.abspath(os.path.join(
            os.path.dirname(FLAGS.input_path), examples[i]['path']))
